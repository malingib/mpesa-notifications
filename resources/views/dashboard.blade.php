...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4 mb-8">
        ...
        <div class="group relative bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl p-6 shadow-xl card-hover overflow-hidden animate-fade-in">
            ...
            <div class="relative z-10">
                ...
                <p class="text-3xl font-bold text-white mb-2">{{ number_format($todayPayments) }}</p>
                <div class="flex items-center text-white/70 text-xs">
                    <i class="fas fa-arrow-up mr-1"></i>
                    <span>KES {{ number_format($todayAmount, 2) }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="group relative bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl p-6 shadow-xl card-hover overflow-hidden animate-fade-in" style="animation-delay: 0.1s">
            ...
            <div class="relative z-10">
                ...
                <p class="text-3xl font-bold text-white mb-2">KES {{ number_format($todayAmount, 2) }}</p>
                <div class="flex items-center text-white/70 text-xs">
                    <i class="fas fa-chart-line mr-1"></i>
                    <span>{{ $todayPayments }} transactions</span>
                </div>
            </div>
        </div>
        ...
        <div class="group relative bg-gradient-to-br from-blue-500 to-cyan-600 rounded-2xl p-6 shadow-xl card-hover overflow-hidden animate-fade-in" style="animation-delay: 0.2s">
            ...
            <div class="relative z-10">
                ...
                <p class="text-3xl font-bold text-white mb-2">{{ number_format($todaySmsSent) }}</p>
                <div class="flex items-center text-white/70 text-xs">
                    <i class="fas fa-percentage mr-1"></i>
                    <span>
                        @if($todayPayments > 0)
                            {{ number_format(($todaySmsSent / $todayPayments) * 100, 1) }}% 
                            ({{ $todaySmsSent }}/{{ $todayPayments }})
                        @else
                            0% (0/0)
                        @endif
                    </span>
        ...
        <div class="group relative bg-gradient-to-br from-pink-500 to-rose-600 rounded-2xl p-6 shadow-xl card-hover overflow-hidden animate-fade-in" style="animation-delay: 0.3s">
            ...
            <div class="relative z-10">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <span class="text-white/80 text-sm font-medium">{{ now()->format('M Y') }}</span>
                </div>
                <h3 class="text-white/80 text-sm font-medium mb-1">This Month</h3>
                <p class="text-3xl font-bold text-white mb-2">{{ number_format($monthPayments) }}</p>
                <div class="flex items-center text-white/70 text-xs">
                    <i class="fas fa-coins mr-1"></i>
                    <span>KES {{ number_format($monthAmount, 2) }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div class="bg-white/80 backdrop-blur-lg rounded-2xl shadow-xl p-6 border border-gray-200/50 animate-fade-in">
            ...
            <div class="space-y-4">
                <div class="flex justify-between items-center p-4 bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl">
                    ...
                    <span class="font-bold text-gray-900 text-lg">{{ number_format($smsStats->total ?? 0) }}</span>
                </div>
                <div class="flex justify-between items-center p-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl">
                    ...
                    <span class="font-bold text-green-600 text-lg">{{ number_format($smsStats->sent ?? 0) }}</span>
                </div>
                <div class="flex justify-between items-center p-4 bg-gradient-to-r from-red-50 to-pink-50 rounded-xl">
                    ...
                    <span class="font-bold text-red-600 text-lg">{{ number_format($smsStats->failed ?? 0) }}</span>
                </div>
                @if(($smsStats->pending ?? 0) > 0)
                <div class="flex justify-between items-center p-4 bg-gradient-to-r from-yellow-50 to-amber-50 rounded-xl">
                    ...
                    <span class="font-bold text-yellow-600 text-lg">{{ number_format($smsStats->pending ?? 0) }}</span>
                </div>
                @endif
                <div class="mt-4 pt-4 border-t border-gray-200">
                    <div class="flex justify-between items-center p-4 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl">
                        ...
                        <span class="font-bold text-indigo-600 text-2xl">
                            {{ $smsStats->total > 0 ? number_format(($smsStats->sent / $smsStats->total) * 100, 1) : 0 }}%
                        </span>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white/80 backdrop-blur-lg rounded-2xl shadow-xl p-6 border border-gray-200/50 animate-fade-in">
            ...
            <div class="space-y-3">
                @forelse($topMerchants as $index => $merchant)
                    <div class="flex justify-between items-center p-4 bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl hover:from-indigo-50 hover:to-purple-50 transition-all duration-200">
                        <div class="flex items-center">
                            ...
                            <div>
                                <p class="font-semibold text-gray-900">{{ strtoupper($merchant->account_type) }} - {{ $merchant->account_number }}</p>
                                <p class="text-sm text-gray-500">{{ $merchant->payments_count }} payments</p>
                            </div>
                        </div>
                        <span class="bg-indigo-100 text-indigo-600 px-3 py-1 rounded-lg font-bold">{{ $merchant->payments_count }}</span>
                    </div>
                @empty
                        ...
                        <tr class="hover:bg-indigo-50 transition-colors duration-150">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    ...
                                    <span class="text-sm font-mono text-gray-900">{{ substr($payment->transaction_id, 0, 20) }}...</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="text-sm font-bold text-gray-900">KES {{ number_format($payment->amount, 2) }}</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $payment->merchant->account_number ?? 'N/A' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @elseif($payment->sms_retry_count > 0)
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800" title="Failed: {{ $payment->sms_error ?? 'Unknown error' }}">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (trendsCtx) {
            const trendsData = {!! json_encode($paymentTrends) !!};
