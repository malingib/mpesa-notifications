    ...
    <div class="bg-white/80 backdrop-blur-lg rounded-2xl shadow-xl border border-gray-200/50 overflow-hidden mb-8 animate-fade-in">
        <div class="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-gray-100">
            <div class="flex flex-wrap items-center justify-between gap-4">
                ...
                <div class="flex items-center space-x-2 bg-gray-200 rounded-lg p-1">
                    <button onclick="switchView('list')" class="view-tab px-4 py-2 rounded-md text-sm font-medium transition-all {{ $viewMode === 'list' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-600 hover:text-gray-900' }}">
                        <i class="fas fa-list mr-2"></i>List View
                    </button>
                    <button onclick="switchView('top_days')" class="view-tab px-4 py-2 rounded-md text-sm font-medium transition-all {{ $viewMode === 'top_days' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-600 hover:text-gray-900' }}">
                        <i class="fas fa-calendar-day mr-2"></i>Top Days
                    </button>
                    <button onclick="switchView('top_months')" class="view-tab px-4 py-2 rounded-md text-sm font-medium transition-all {{ $viewMode === 'top_months' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-600 hover:text-gray-900' }}">
                        <i class="fas fa-calendar-alt mr-2"></i>Top Months
                    </button>
                </div>

                <!-- Date Filters -->
                <form method="GET" action="{{ route('payments.index') }}" class="flex items-center space-x-3 flex-wrap">
                    ...
                    @if($dateFrom || $dateTo)
                        <a href="{{ route('payments.index', ['view' => $viewMode]) }}" class="bg-gray-500 text-white px-4 py-2 rounded-lg hover:bg-gray-600 transition-colors text-sm font-medium">
                        ...
                        @forelse($payments as $payment)
                            <tr class="hover:bg-indigo-50 transition-colors duration-150 payment-row" data-status="{{ strtolower($payment->status) }}">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="w-10 h-10 bg-gradient-to-br from-indigo-500 to-purple-500 rounded-full flex items-center justify-center mr-3 text-white font-semibold text-xs">
                                            {{ strtoupper(substr($payment->merchant->account_number ?? 'N', 0, 1)) }}
                                        </div>
                                        <div>
                                            <div class="text-sm font-medium text-gray-900">{{ $payment->merchant->account_number ?? 'N/A' }}</div>
                                            <div class="text-xs text-gray-500">{{ ucfirst($payment->merchant->account_type ?? 'N/A') }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-mono text-gray-900">{{ substr($payment->transaction_id, 0, 20) }}...</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">{{ $payment->payer_name ?? 'N/A' }}</div>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    @elseif($payment->sms_retry_count > 0)
                                        <span class="sms-status inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800" title="Failed: {{ $payment->sms_error ?? 'Unknown error' }}">
                                            <i class="fas fa-times-circle mr-1"></i>Failed
                                        </span>
                                    @else
        ...
        @elseif($viewMode === 'top_days')
            ...
            <div class="p-6">
                ...
                <div class="space-y-3">
                    @forelse($topRevenueData as $index => $day)
                        <div class="flex items-center justify-between p-4 bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl hover:from-indigo-50 hover:to-purple-50 transition-all duration-200">
                            <div class="flex items-center">
                                ...
                                <div>
                                    <p class="font-semibold text-gray-900">{{ $day['date'] }}</p>
                                    <p class="text-sm text-gray-500">{{ number_format($day['count']) }} transaction{{ $day['count'] != 1 ? 's' : '' }}</p>
                                </div>
                            </div>
                            <div class="text-right">
                                <p class="font-bold text-green-600 text-lg">KES {{ number_format($day['total'], 2) }}</p>
        ...
        @elseif($viewMode === 'top_months')
            ...
            <div class="p-6">
                ...
                <div class="space-y-3">
                    @forelse($topRevenueData as $index => $month)
                        <div class="flex items-center justify-between p-4 bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl hover:from-indigo-50 hover:to-purple-50 transition-all duration-200">
                            <div class="flex items-center">
                                ...
                                <div>
                                    <p class="font-semibold text-gray-900">{{ $month['label'] }}</p>
                                    <p class="text-sm text-gray-500">{{ number_format($month['count']) }} transaction{{ $month['count'] != 1 ? 's' : '' }}</p>
                                </div>
                            </div>
                            <div class="text-right">
                                <p class="font-bold text-green-600 text-lg">KES {{ number_format($month['total'], 2) }}</p>
                                <p class="text-xs text-gray-500">Revenue</p>
                            </div>
                        </div>
                    @empty
...
<script>
    ...
    function calculateEarnings(period) {
        ...
        fetch('{{ route("payments.calculate-earnings") }}', {
            method: 'POST',
            headers: {
