        ...
        <div class="bg-white/80 backdrop-blur-lg rounded-2xl shadow-xl p-6 border border-gray-200/50 animate-fade-in">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900">{{ number_format($totalMerchants ?? 0) }}</p>
                </div>
        ...
        <div class="bg-white/80 backdrop-blur-lg rounded-2xl shadow-xl p-6 border border-gray-200/50 animate-fade-in" style="animation-delay: 0.1s">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-green-600">{{ number_format($activeMerchants ?? 0) }}</p>
                </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200" id="merchantsTableBody">
                    @forelse($merchants as $merchant)
                        <tr class="hover:bg-indigo-50 transition-colors duration-150 merchant-row" data-status="{{ $merchant->is_active ? 'active' : 'inactive' }}">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="w-10 h-10 bg-gradient-to-br from-blue-500 to-cyan-500 rounded-full flex items-center justify-center mr-3 text-white font-semibold text-xs">
                                        {{ strtoupper(substr($merchant->account_number, 0, 1)) }}
                                    </div>
                                    <div class="text-sm font-medium text-gray-900 font-mono">{{ $merchant->account_number }}</div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800">
                                    {{ ucfirst($merchant->account_type) }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ number_format($merchant->payments_count ?? 0) }} (last 30 days)
                            </td>
