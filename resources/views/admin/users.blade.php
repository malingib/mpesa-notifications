        ...
        <div class="bg-white/80 backdrop-blur-lg rounded-2xl shadow-xl p-6 border border-gray-200/50 animate-fade-in" style="animation-delay: 0.2s">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-blue-600">{{ $totalClients ?? 0 }}</p>
                </div>
        ...
        <div class="bg-white/80 backdrop-blur-lg rounded-2xl shadow-xl p-6 border border-gray-200/50 animate-fade-in" style="animation-delay: 0.3s">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-purple-600">{{ $totalAdmins ?? 0 }}</p>
                </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200" id="usersTableBody">
                    @forelse($users as $user)
                        <tr class="hover:bg-indigo-50 transition-colors duration-150 user-row" data-role="{{ $user->role }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ number_format($user->payments_count ?? 0) }}
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center space-x-2">
                                    ...
                                    @if(!$user->isAdmin())
                                        <form action="{{ route('admin.impersonate', $user->id) }}" method="POST" class="inline" onsubmit="return confirm('Login as {{ $user->name }}?')">
                                            @csrf
